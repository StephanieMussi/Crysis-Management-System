<?php
    ...
    class cmoController
    {
        ...
        */
        ...
        public function getAllSevereIncidents(){
            //query to get all incidents
            include ($this->scriptname);

            $sql = "SELECT * FROM incident WHERE IsSevere = 'true' AND status = 'open'";
        ...
        public function getReportIncidents($timeinterval)
        {
            include($this->scriptname);
            //query to get all incidents
            $sql = "SELECT * FROM `incident` WHERE status = 'open' OR TIMESTAMPDIFF(MINUTE,statusDateTime,CURRENT_TIMESTAMP) < ?;";

            $stmt = mysqli_stmt_init($conn);

            mysqli_stmt_prepare($stmt, $sql);

            mysqli_stmt_bind_param($stmt, "s", $timeinterval);
        ...
        public function getNotifyNewIncident()
        {
            include($this->scriptname);
            ...
            if($result2 != null) {
                ...
                if ($resultCheck != 0) {
                    $sql2 = "UPDATE incident SET HasNotify = 'true' WHERE incidentId =" . $dbdata[0]['incidentId'] . ";";
        ...
        public function getResponsePMO()
        {
            include($this->scriptname);
        ...
        public function getUpdateAgency()
        {
            include($this->scriptname);
            $sql = "SELECT * FROM updates WHERE HasNotify LIKE 'false%';";
            $result2 = mysqli_query($conn, $sql);
            ...
            $resultCheck = mysqli_num_rows($result2);
            if ($resultCheck != 0) {
                $sql2 = "UPDATE updates SET HasNotify = 'true' WHERE incidentId =" . $dbdata[0]['incidentId'] . ";";
                $result = mysqli_query($conn, $sql2);
            }
            return $dbdata;
        }

        //For cmo dashboard pie chart: severecase vs non-severecase
        public function getPercentageOfNonsevereCase(){
            include($this->scriptname);
            ...
            while ($resultCheck = mysqli_fetch_assoc($result)) {
                $incidentCount = $resultCheck['COUNT(*)'];
            }
                    
                    
            $sql = "SELECT COUNT(*) FROM severeincident";
            //run query
            $result = mysqli_query($conn, $sql);
            ...
            while ($resultCheck = mysqli_fetch_assoc($result)) {
                $severecaseCount = $resultCheck['COUNT(*)'];
            }
            $nonseverecaseCount = $incidentCount - $severecaseCount;
            $nonseverecasePercentage = ($nonseverecaseCount/$incidentCount)*100;
            return $nonseverecasePercentage;
                    
        }
                
        public function getPercentageOfSevereCase(){
            include($this->scriptname);
            $sql = "SELECT COUNT(*) FROM severeincident";
            //run query
            $result = mysqli_query($conn, $sql);
            ...
            $severecasePercentage = ($severecaseCount/$incidentCount)*100;
            return $severecasePercentage;
        }
